//! The `.kir` project container, autosave, and the crash-recovery journal —
//! docs/10-FILE-FORMAT.md, Phase 0 scope (no thumbnails yet).

use kiriko_core::ops::Op;
use kiriko_core::Document;
...
pub const FORMAT: &str = "kiriko-project";
...
impl Manifest {
    fn current() -> Self {
        Self {
            ...
            written_by: format!("kiriko {}", env!("CARGO_PKG_VERSION")),
...
pub fn save(doc: &Document, path: &Path) -> Result<(), ProjectError> {
    ...
    let tmp = dir.join(format!(
        ".{}.tmp-{}",
        stem.unwrap_or_else(|| "project.kir".into()),
        std::process::id()
    ));
...
/// Open a `.kir` file. Unknown JSON fields survive via the model's `extra` maps.
pub fn open(path: &Path) -> Result<(Document, Manifest), ProjectError> {
    ...
    let manifest: Manifest = {
        let mut entry = zip
            ...
            .map_err(|_| ProjectError::NotAKirikoProject)?;
...
pub fn autosave(doc: &Document, project_path: &Path, keep: usize) -> Result<PathBuf, ProjectError> {
    ...
    let slot = |k: usize| dir.join(format!("{stem}.autosave-{k}.kir"));
...
mod tests {
    use super::*;
    use kiriko_core::model::{FootageItem, MediaRef, ProjectItem};
    use kiriko_core::ops::apply;
    ...
    fn save_open_round_trip_and_no_temp_litter() {
        ...
        let path = dir.path().join("edit.kir");
    ...
    fn unknown_fields_survive_open_save_open() {
        ...
        // A "newer Kiriko" adds fields this version knows nothing about.
    ...
    fn too_new_projects_are_refused_clearly() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("future.kir");
        let file = File::create(&path).unwrap();
        let mut zip = ZipWriter::new(file);
        let opts = SimpleFileOptions::default();
        zip.start_file("manifest.json", opts).unwrap();
        zip.write_all(
            br#"{"format":"kiriko-project","schema_version":"9.0.0","written_by":"kiriko 9","min_reader":"9.0.0"}"#,
        )
        .unwrap();
    ...
    fn autosave_rotates_and_keeps_n() {
        let dir = tempfile::tempdir().unwrap();
        let project = dir.path().join("edit.kir");
        for i in 0..7u32 {
            let mut doc = Document::new();
        ...
        let (newest, _) = open(&autos.join("edit.autosave-1.kir")).unwrap();
        assert_eq!(newest.extra["gen"], serde_json::json!(6));
        let (oldest, _) = open(&autos.join("edit.autosave-5.kir")).unwrap();
