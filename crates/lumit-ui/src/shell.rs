...
#[cfg(feature = "media")]
impl LayerMap {
    fn of(layer: &lumit_core::model::Layer, lt: f64, draw: egui::Rect, scale: f32) -> Self {
...
fn follow_edit(app: &mut AppState, op: &lumit_core::Op) {
    match op {
...
fn prop_unit(prop: lumit_core::model::TransformProp) -> &'static str {
...
fn key_shape(k: &lumit_core::anim::Keyframe) -> KeyShape {
...
fn side_influence(side: lumit_core::anim::SideInterp) -> f64 {
    match side {
...
fn side_speed(side: lumit_core::anim::SideInterp) -> Option<f64> {
    match side {
...
fn fit_values_with_handles(keys: &[lumit_core::anim::Keyframe]) -> (f64, f64) {
    let mut lo = f64::MAX;
    let mut hi = f64::MIN;
...
fn nudge_selected_values(keys: &mut [lumit_core::anim::Keyframe], selection: &[usize], delta: f64) {
    for &i in selection {
...
fn blend_name(b: lumit_core::model::BlendMode) -> &'static str {
...
fn layer_keyframe_times(layer: &lumit_core::model::Layer) -> Vec<f64> {
    use lumit_core::anim::Animation;
...
struct RowCtx<'a> {
    theme: &'a Theme,
    comp_id: uuid::Uuid,
    layer: &'a lumit_core::model::Layer,
    lt: f64,
    off: f64,
    fps: f64,
...
/// editor's value lens.
#[cfg(feature = "media")]
fn layer_source_fps(app: &AppState, layer: &lumit_core::model::Layer, fallback_fps: f64) -> f64 {
    use lumit_core::model::LayerKind;
...
fn layer_source_fps(_app: &AppState, _layer: &lumit_core::model::Layer, fallback_fps: f64) -> f64 {
    fallback_fps
}
...
fn rational_at(seconds: f64) -> lumit_core::Rational {
