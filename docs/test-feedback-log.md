...
- [~] T14 Combined X/Y row DONE: any effect `<base>_x`/`<base>_y` Float pair now renders as ONE row (shared stopwatch keying both, shared ◄ ◆ ► over the union, two boxes) in both the Effect Controls panel and the timeline (they share `effects_rows`). Applies automatically to Radial blur Centre and the Transform effect's Anchor/Position/Scale. REMAINING: the viewfinder pixel-picker (click the Viewer to set x/y like the dropper) — interaction-heavy, best done with preview access.
